...
### Navigation Flow
1. **ImportPage** (`/`): User selects PDF → computes SHA-256 hash (first 24 chars = pdfId) → stores File in PdfContext → navigates to `/reader/:pdfId`
...
- `ltp_mvp::{pdfId}::onboarding_seen` → `'1'` (string flag indicating onboarding completed)

### Stamp Types & UI Components

**Four Stamp Categories** ([StampToolbar.jsx](src/components/StampToolbar.jsx)):
- `rhythm` (♪) - 节奏标记 (Rhythm) - Orange (#f59e0b)
  - Shows badge with "steps×repeats" (e.g., "4×3")
  - Gold border (3px) to distinguish from other types
  - Tooltip shows detailed payload info
- `form` (□) - 形态标记 (Form) - Blue (#3b82f6)  
- `tactile` (✋) - 触觉标记 (Tactile) - Purple (#8b5cf6)
- `generic` (●) - Default fallback - Green (#10b981)
...
- Example from [ReaderPage.jsx](src/pages/ReaderPage.jsx#L218-L220):
  ...
  <button onClick={handleAddStamp}>＋ 添加标记</button>
  ```
...
```js
const left = stamp.x * stageWidth   // Convert 0.85 → 850px (if width=1000px)
